...
exports.handler = async (event, context) => {
  ...
  try {
    connectionData = await ddb.scan({ TableName: TABLE_NAME, ProjectionExpression: 'connectionId, endpoint' }).promise();
  } catch (e) {
  ...
  let recs = [];
  if(event.Records && event.Records.length >0){ //messages from sqs
    recs = event.Records
  }else if(event.body){ //direct message from api gateway
    recs.push(event)
  }
  ...
  const postCalls = connectionData.Items.map(async ({ connectionId, endpoint }) => {
    try {
      for (const postDataItem of postData) {
        await getApi(endpoint).postToConnection({ ConnectionId: connectionId, Data: postDataItem }).promise();
      }
    } catch (e) {
      if (e.statusCode === 410) {
